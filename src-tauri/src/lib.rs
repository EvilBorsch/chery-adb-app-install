...
fn check_device(app: AppHandle) -> Result<DeviceInfo> {
    ...
    let serial = run_output(&adb, ["get-serialno"])?.stdout.trim().to_string();
...
#[tauri::command]
fn install_apk(app: AppHandle, apk_path: String) -> Result<InstallResult> {
    ...
    let install_cmd =
        format!("CLASSPATH={remote_helper} /system/bin/app_process /system/bin LocalInstall {remote_apk}");
    ...
    run_optional(&adb, ["shell", "am", "force-stop", package_name.as_str()], &mut log);
    run_optional(
        &adb,
        [
            "shell",
...
fn grant_permissions_inner(adb: &Path, package_name: &str, log: &mut WorkLog) -> Result<()> {
    ...
    for permission in requested_permissions(adb, package_name)? {
        let output = run_output(adb, ["shell", "pm", "grant", package_name, permission.as_str()])?;
...
fn detect_package_with_aapt(apk_path: &Path) -> Result<String> {
    ...
    if !output.status.success() {
        return Err(InstallerError::Message("aapt не смог прочитать APK".to_string()));
    }
...
fn resource_localinstall(app: &AppHandle) -> Result<PathBuf> {
    app.path()
        .resolve("resources/localinstall.apk", tauri::path::BaseDirectory::Resource)
...
fn tools_dir(app: &AppHandle) -> Result<PathBuf> {
    let base = app
        .path()
        .app_data_dir()
        .map_err(|err| InstallerError::Message(format!("Не удалось получить app data dir: {err}")))?;
